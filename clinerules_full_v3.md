...
# 八、整合版概念說明（BASELINE / BACKFILL / SPEC / PROPOSAL / CHANGE / TASK）

## 1. project_baseline（專案基準）
描述：
...
## 2. backfill（回填規格）
當功能已存在但 spec 缺失，backfill 將現況寫入 ADDED Requirements。

---

## 3. spec（規格文件 / Requirements）
系統應該如何運作的正式文件，包含 Requirement + Scenario。  
是實作、測試、審查的唯一真實來源。
...
proposal（為何要新增 / 修改）
        │
        ▼
change（完整封裝：proposal + spec delta + tasks）
        │
        ▼
task（實作 / 驗證）
```
...
所有規格/變更/基準資料 → **統一放 openspec/**  

```text
.
├── .clinerules                      # 本檔案
├── openspec/
│   ├── project_baseline.md          # 對應 project_baseline
│   ├── specs/                       # 穩定且正式的規格
│   │   └── <capability-name>/
│   │       └── spec.md
│   ├── changes/                     # 所有變更（change unit）
│   │   └── <change-id>/
│   │       ├── proposal.md
│   │       ├── tasks.md
│   │       └── specs/
